...
from PyQt5.QtWidgets import QMessageBox, QTableWidgetItem, QSpinBox, QCheckBox, QMainWindow, QApplication, QDialog, \
    QPushButton
from models import *
...
class OrderSubEditWindow(QMainWindow):
    def __init__(self, order_id, order_signal, parent=None):
        super().__init__(parent)
        uic.loadUi('ui/order_subedit_window.ui', self)
        order = Order.get(Order.id == order_id)
        self.done_checkBox.setChecked(order.is_completed)
        self.payed_checkBox.setChecked(order.is_paid)
        self.order_signal = order_signal
        self.load_table_by_id(order)
        self.load_dish_by_id(order)
        self.order = order
        self.edit_order_pushButton.clicked.connect(self.edit_order)

    def load_table_by_id(self, order):
        ...
        if order and order.table:
            index = self.table_comboBox.findData(order.table.id)
            if index >= 0:
                self.table_comboBox.setCurrentIndex(index)

    def load_dish_by_id(self, order):
        ...
        row = 0
        order_items = {item.dish.id: item.quantity for item in OrderItem.select().where(OrderItem.order == order)}
        ...
        for dish in Dish.select():
            ...
            spinbox = QSpinBox()
            spinbox.setMinimum(order_items.get(dish.id, 0))
    ...
    def edit_order(self):
        try:
            ...
            order.table = selected_table_id
            order.is_completed = self.done_checkBox.isChecked()
            order.is_paid = self.payed_checkBox.isChecked()
            order.save()
            ...
            if total_quantity == 0:
                QMessageBox.warning(self, "Внимание",
                                    "Заказ не может быть создан без добавления блюд в количестве больше нуля.")
                return

            for dish_id, quantity in dish_quantities.items():
                dish = Dish.get_by_id(dish_id)
                try:
                    order_item = OrderItem.get(OrderItem.order == order, OrderItem.dish == dish)
                    order_item.quantity = quantity
                    order_item.save()
                except DoesNotExist:
                    OrderItem.create(order=order, dish=dish, quantity=quantity)
...
class OrderEditWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.edit_pushButton.clicked.connect(self.edit_order)
    ...
    def edit_order(self):
        order_id = self.order_comboBox.currentData()
        self.sub_edit_window = OrderSubEditWindow(order_id, self.order_updated)
        self.sub_edit_window.show()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, current_waiter):
        ...
        self.tableWidget.setHorizontalHeaderLabels(
            ["ID", "Блюда", "Общая стоимость", "Выполнен", "Оплачен", "Официант", "Столик", "Дата"])
        self.load_orders()
